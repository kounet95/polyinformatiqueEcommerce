...
export class CustomerService {
  ...
  getCustomerByEmail(email: string): Observable<CustomerEcommerceDTO> {
  return this.http.get<CustomerEcommerceDTO>(`${this.queryBase}/by-email/${email}`);
  }
  ...
  getAddressesByCustomerId(customerId: string): Observable<AddressDTO[]> {
    return this.http.get<AddressDTO[]>(`http://localhost:8888/ecpolyquery/api/address/by-customer/${customerId}`);
  }
