...
public class OrderAggregate {
  ...
  public void handle(GenerateInvoiceCommand cmd) {
    if (!this.confirmed) throw new IllegalStateException("Cannot generate invoice for unconfirmed order.");
    if (this.paid) throw new IllegalStateException("Invoice already paid.");
  ...
  public void handle(PayInvoiceCommand cmd) {
    if (!this.confirmed) throw new IllegalStateException("Cannot pay invoice for unconfirmed order.");
    if (this.paid) throw new IllegalStateException("Invoice already paid.");

    log.info("[AGGREGATE] PayInvoiceCommand received: {}", cmd);
    apply(new InvoicePaidEvent(cmd.getId(), cmd.getInvoiceDTO(), cmd.getPaymentIntentId(), cmd.getSessionId()));
  }

  @CommandHandler
  public void handle(StartShippingCommand cmd) {
    if (!this.paid) throw new IllegalStateException("Cannot start shipping unpaid order.");
  ...
  public void handle(DeliverOrderCommand cmd) {
    if (!this.shipped) throw new IllegalStateException("Cannot deliver order that hasn't been shipped.");
  ...
  public void handle(CancelOrderCommand cmd) {
    if (this.delivered) throw new IllegalStateException("Cannot cancel a delivered order.");
    if (this.paid) throw new IllegalStateException("Cannot cancel a paid order.");
