...
public class AddressQueryHandler {
  ...
  /**
   * Handler Axon pour GetAddressesByCustomerIdQuery
   */
  @QueryHandler
  public List<AddressDTO> handle(GetAddressesByCustomerIdQuery query) {
    log.debug("Handling GetAddressesByCustomerIdQuery for customerId={}", query.getCustomerId());
    ...
    return links.stream()
      ...
      .map(address -> {
        address.getId(); // force chargement proxy
