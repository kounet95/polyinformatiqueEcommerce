...
public class AddressController {
  ...
  /**
   * REST endpoint pour récupérer les adresses d’un client
   * Exemple : GET /api/query/address/by-customer/abc-123
   */
  @GetMapping("/by-customer/{customerId}")
  @PreAuthorize("hasAuthority('USER')")
  public CompletableFuture<List<AddressDTO>> getAddressesByCustomerId(@PathVariable String customerId) {
    GetAddressesByCustomerIdQuery query = new GetAddressesByCustomerIdQuery(customerId);
    return queryGateway.query(query, ResponseTypes.multipleInstancesOf(AddressDTO.class));
  }
